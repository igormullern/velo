...
export function createOrderLookupActions(page: Page) {
  ...
  const validateStatusBadgeInternal = async (status: OrderStatus) => {
  ...
  return {
    ...
    async validateStatusBadge(status: OrderStatus) {
      await validateStatusBadgeInternal(status)
    },

    async assertOrderDetails(order: OrderDetails) {
      ...
      await validateStatusBadgeInternal(order.status)
    },
...
export type OrderLookupActions = ReturnType<typeof createOrderLookupActions>
